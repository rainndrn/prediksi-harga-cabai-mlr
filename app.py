...
Dashboard ini merupakan implementasi model Multiple Linear Regression yang dibangun pada penelitian skripsi.

Model menggunakan variabel **hari_ke**, **lag-1**, **lag-3**, **lag-7**, dan **Moving Average 7 Hari (MA7)**.

Prediksi hanya dapat dilakukan pada tanggal yang memiliki data historis yang diperlukan untuk membentuk variabel tersebut. Dashboard belum dapat melakukan prediksi pada tanggal di luar rentang data penelitian.

Pengembangan lebih lanjut dapat dilakukan dengan mengintegrasikan dashboard dengan sumber data harga harian sehingga pembentukan variabel historis dapat dilakukan secara otomatis.
...
if selected == "Prediksi":
    ...
    tanggal = st.selectbox(
        ...
        df["tanggal_lengkap"].dt.strftime("%Y-%m-%d")
